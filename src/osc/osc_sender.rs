...
pub fn packet_to_buffer(packet: OscPacket) -> Vec<u8> {
	...
	match packet {
		OscMessage{ addr: addr, args: args} => {
			...
			buf.write_str(to_osc_string(addr));
...
fn write_arg(buf: &mut MemWriter, arg: OscArg) {
	match arg {
		...
		OscStr(v) 	=> { buf.write_str(to_osc_string(v.to_owned())); },
		OscBlob(v) 	=> {
...
fn test_to_osc_string() {
	...
	let t0 = ~"a";
	assert_eq!(~"a\0\0\0",to_osc_string(t0));

	let t1 = ~"hello";
	assert_eq!(~"hello\0\0\0",to_osc_string(t1));

	let t2 = ~"";
	assert_eq!(~"\0\0\0\0",to_osc_string(t2));
}
