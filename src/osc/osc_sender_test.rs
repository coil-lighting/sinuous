...
fn main() {
	...
	let tests = vec!(
		OscMessage{
			addr: ~"/test/addr/1",
			args: vec!( OscInt(123), OscFloat(2.0), OscStr(~"I'm a string"), OscBlob(vec!(5u8, 10u8, 15u8)) )
		},
		OscMessage{
			addr: ~"/test/addr/2",
		...
		OscBundle{
			time_tag: (123,456),
			conts: vec!( OscMessage{addr: ~"/subaddr", args: vec!(OscInt(789)) } )
		},
		OscBundle{
			time_tag: (789,1001),
			conts: vec!(
				OscMessage{addr: ~"/subaddr", args: vec!(OscInt(789)) },
				OscBundle{
					time_tag: (1,0),
					conts: vec!( OscMessage{addr: ~"/subsubaddr", args: vec!(OscBlob(vec!(5u8, 10u8, 15u8, 20u8, 25u8)))})
				}
			)
		}
	);
