...
fn read_null_term_string(reader: &mut BufReader) -> IoResult<~str> {
	// read until null
...
fn test_read_bundle(){
	...
	let packet = OscBundle{
		time_tag: (0,1),
		conts: vec!(
			OscMessage{ addr:~"/t", args: vec!(a1)},
			OscBundle{
				time_tag: (123,456),
				conts: vec!(
					OscMessage{ addr:~"/a", args: vec!(a2, a3)},
					OscMessage{ addr:~"/b", args: vec!(a4)}
				)
			}
		)
	};
...
fn test_read_null_term_string(){
	...
	assert_eq!(read_null_term_string(&mut reader),Ok(~"abc"));

	// multiple nulls and multiple calls
	let buf2 = ~[97u8, 98u8, 0u8, 0u8, 99u8, 0u8];
	reader = BufReader::new(buf2.as_slice());
	assert_eq!(read_null_term_string(&mut reader),Ok(~"ab"));
	assert_eq!(read_null_term_string(&mut reader),Ok(~"c"));
	...
	assert_eq!(read_null_term_string(&mut reader),Ok(~""));
	assert!(read_null_term_string(&mut reader).is_err());
