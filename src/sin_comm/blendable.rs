...
pub enum Blendable {
	...
	Set(int,(int,int)), // set of possible integer values between min and max, defined by tuple (inclusive)
...
impl Blendable {
	...
	pub fn blend(&self, bw: Blendable, mode: BlendMode) -> Blendable {
		FlUni(0.)
	}
	...
	pub fn to_FlUni(&self) -> Blendable {
		match *self {
			...
			ColorRGB(r,g,b) => FlUni ( (std::cmp::max( r, std::cmp::max(g,b) ) as f64) / 255. ) // for now use Value
		}
	}
	...
	pub fn to_Set(&self, min: int, max: int) -> Blendable {
		match *self {
			Set(_,(min1,max1)) if min == min1 && max == max1 => *self,
			to_coerce => {
				let mut val = (to_coerce.to_FlUni().unwrap_FlUni() * ((max - min + 1) as f64)).floor() as int;
				if val > max {
					val = max;
				}
				Set( val, (min,max) )
	...
	pub fn to_ColorRGB(&self) -> Blendable {
		match *self {
			ColorRGB(_,_,_) => *self,
			to_coerce => {
				let mut val = (to_coerce.to_FlUni().unwrap_FlUni() * 256.).floor() as int;
				if val > 255 {
					val = 255;
				}
